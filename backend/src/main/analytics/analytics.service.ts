  ...
  async getDayAnalisis(fecha, businessId, userId) {
    try {
      ...
      SELECT
          ...
          dh.fecha_consulta,
          TO_CHAR(dh.fecha_consulta, 'DD/MM/YYYY') as fecha_formateada,
          TO_CHAR(dh.fecha_consulta, 'Day') as dia_semana,
          ...
          CASE
              ...
              ELSE (dh.ventas_hoy - dh.gastos_hoy)::text
          ...
          CASE 
              WHEN dh.transacciones_ingresos_hoy > 0 THEN
                  CASE
                      ...
                      ELSE ROUND(dh.ventas_hoy / dh.transacciones_ingresos_hoy, 2)::text
                  END
              ELSE '0'
      ...
      SELECT
        ...
        CASE 
          ...
          ELSE 
            SUM(dt.subtotal)::text
        END as ingresos_formatted,
        (SELECT TO_CHAR(fecha_consulta, 'DD/MM/YYYY') FROM fecha_input) as fecha_formateada,
        (SELECT TO_CHAR(fecha_consulta, 'Day') FROM fecha_input) as dia_semana,
        p.precio_unitario,
        p.costo_unitario,
    ...
    try {
      ...
      const responsePerformance = await this.executeQuery(
        ...
        lunes_semana AS (
          SELECT
            fecha_consulta,
            date_trunc('week', fecha_consulta)::date as lunes_fecha
          FROM fecha_input
        ),
        ingresos_por_dia AS (
          SELECT
        ...
        SELECT
          dias.dia_numero,
          (ls.lunes_fecha + (dias.dia_numero - 1)) as fecha_dia,
          dias.nombre_dia,
          TO_CHAR((ls.lunes_fecha + (dias.dia_numero - 1)), 'Day') as nombre_dia_db,
          TO_CHAR((ls.lunes_fecha + (dias.dia_numero - 1)), 'DD/MM/YYYY') as fecha_formateada,
          ...
          CASE
            WHEN (ls.lunes_fecha + (dias.dia_numero - 1)) = (SELECT fecha_consulta FROM fecha_input) THEN 'SÍ'
            ELSE 'NO'
        ...
        LEFT JOIN ingresos_por_dia ipd ON ipd.fecha_dia = (ls.lunes_fecha + (dias.dia_numero - 1))
        LEFT JOIN egresos_por_dia epd ON epd.fecha_dia = (ls.lunes_fecha + (dias.dia_numero - 1))
        LEFT JOIN productos_vendidos_por_dia pvpd ON pvpd.fecha_dia = (ls.lunes_fecha + (dias.dia_numero - 1))
        ...
        `WITH
              ...
              lunes_semana AS (
                SELECT
                  fecha_consulta,
                  date_trunc('week', fecha_consulta)::date as lunes_fecha,
                  (date_trunc('week', fecha_consulta)::date + 6) as domingo_fecha
                FROM fecha_input
              )
              SELECT
                p.id as producto_id,
                ...
                CASE 
                  ...
                  ELSE 
                    SUM(dt.subtotal)::text
                END as ingresos_formatted,
                (SELECT TO_CHAR(lunes_fecha, 'DD/MM/YYYY') FROM lunes_semana) as inicio_semana,
                (SELECT TO_CHAR(domingo_fecha, 'DD/MM/YYYY') FROM lunes_semana) as fin_semana
              FROM productos p
              INNER JOIN detalle_transacciones dt ON p.id = dt.producto_id
              INNER JOIN transacciones t ON dt.transaccion_id = t.id
              INNER JOIN puntos_venta pv ON t.punto_venta_id = pv.id
      ...
      const responsePerformance = await this.executeQuery(
        ...
        WITH RECURSIVE
          fecha_input AS (
              SELECT
                  DATE(?) as fecha_consulta,
                  date_trunc('month', DATE(?))::date as fecha_inicio_intervalo,
                  (date_trunc('month', DATE(?))::date + interval '1 month' - interval '1 day')::date as fecha_fin_intervalo
          ),
          params AS (
              SELECT
          ...
          primer_lunes AS (
              SELECT
                  fecha_inicio_intervalo,
                  fecha_fin_intervalo,
                  (fecha_inicio_intervalo + ((8 - EXTRACT(ISODOW FROM fecha_inicio_intervalo))::int % 7)) as primer_lunes_fecha
              FROM params
          ),
          semanas AS (
              SELECT
                  ...
                  (primer_lunes_fecha + 6) as fecha_fin_semana,
                  fecha_fin_intervalo
              FROM primer_lunes
              ...
              SELECT
                  ...
                  (fecha_inicio_semana + 7),
                  (fecha_fin_semana + 7),
                  fecha_fin_intervalo
              FROM semanas
              WHERE (fecha_inicio_semana + 7) <= fecha_fin_intervalo
          ),
          ...
          SELECT
              ...
              s.fecha_fin_semana,
              TO_CHAR(s.fecha_inicio_semana, 'DD/MM/YYYY') as fecha_inicio_formateada,
              TO_CHAR(s.fecha_fin_semana, 'DD/MM/YYYY') as fecha_fin_formateada,
              CASE
                  WHEN CURRENT_DATE BETWEEN s.fecha_inicio_semana AND s.fecha_fin_semana THEN 'SÍ'
                  ELSE 'NO'
      ...
      const responseBestSellers = await this.executeQuery(
        `WITH
            fecha_input AS (
              SELECT
                DATE(?) as fecha_consulta,
                date_trunc('month', DATE(?))::date as fecha_inicio_mes,
                (date_trunc('month', DATE(?))::date + interval '1 month' - interval '1 day')::date as fecha_fin_mes
            )
            SELECT
              p.id as producto_id,
              p.nombre as producto_nombre,
              ...
              CASE 
                ...
                ELSE 
                  SUM(dt.subtotal)::text
              END as ingresos_formatted,
              (SELECT TO_CHAR(fecha_inicio_mes, 'DD/MM/YYYY') FROM fecha_input) as inicio_mes,
              (SELECT TO_CHAR(fecha_fin_mes, 'DD/MM/YYYY') FROM fecha_input) as fin_mes,
              (SELECT TO_CHAR(fecha_inicio_mes, 'Month') FROM fecha_input) as nombre_mes,
              (SELECT EXTRACT(YEAR FROM fecha_inicio_mes) FROM fecha_input) as año_mes
            FROM productos p
            INNER JOIN detalle_transacciones dt ON p.id = dt.producto_id
            INNER JOIN transacciones t ON dt.transaccion_id = t.id
            INNER JOIN puntos_venta pv ON t.punto_venta_id = pv.id
