...
export class TransactionsService {
    ...
    try {
      ...
      if (newTransaction.tipo === 'egreso') {
        ...
        const [result]: any = await connection.query(
          'INSERT INTO transacciones (punto_venta_id, tipo, fecha, monto_total, categoria_id, usuario_id, concepto) VALUES (?, ?, ?, ?, ?, ?, ?) RETURNING id',
          [
        ...
        const [productsRows]: [any[], any] = await connection.query(
          'SELECT p.id, p.precio_unitario, p.nombre, p.unidad_medida FROM productos p ' +
            ...
            'JOIN puntos_venta pv ON pv.negocio_id = n.id ' +
            'WHERE p.id = ANY(?) AND pv.id = ? AND n.propietario = ?',
        ...
        const [result]: any = await connection.query(
          'INSERT INTO transacciones (punto_venta_id, tipo, fecha, monto_total, usuario_id) VALUES (?, ?, ?, ?, ?) RETURNING id',
          [
            newTransaction.punto_venta_id,
            newTransaction.tipo,
            newTransaction.fecha,
  ...
  async getTransactionByDay(
    ...
    try {
      ...
      LEFT JOIN detalle_transacciones dt ON t.id = dt.transaccion_id AND t.tipo = 'ingreso'
      ...
      transactionData.forEach((row) => {
        ...
        // Solo agregar detalles si es ingreso y hay datos de detalle
